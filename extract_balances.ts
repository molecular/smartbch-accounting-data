...
sbch.blockNumber().then(async (latest: string) => {
	...
	//let contract_address = "0xcaaebae23406eaa164c0ec1621c04a073d7f6f43" // MISTswap LP WBCH-CAO
	//let contract_address = "0x674a71e69fe8d5ccff6fdcf9f1fa4262aa14b154" // MISTswap LP WBCH-MIST
...
async function get_balances(accounts_filename, contract) {
	const parser = parse({delimiter: ';', from_line: 2}, function(err, data){
		...
		.then((accounts: string[]) => { 
			...
			return Promise.all(
				accounts.map((account) => {
					...
					// 	null,
					// 	contract.address,
					// 	Web3.utils.sha3("balanceOf(address)")?.substring(0,10) + util.convertAddressToTopic(account).substring(2) // TODO use web3-eth-abi encodeParameter ?
					// )
					return sbch.call(
						null, // from: string | null
						contract.address, // to: string | null
						"balanceOf(address)", // method_signature: string
						[ { type: 'address', value: account } ], // parameters: TypedParamater[]
						// returnType?: string | NamedReturnType[]
					)
					.catch(logToConsole)
				})
		...
		.then((result) => {
			//console.log("balanceOf(address) results", result)
